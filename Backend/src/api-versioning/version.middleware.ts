...
export class VersionMiddleware implements NestMiddleware {
    ...
    try {
      ...
      if (!version) {
        ...
        if (defaultVersion) {
          ...
          this.logger.debug(`Using default version: ${this.versioningService.getVersionString(defaultVersion)}`);
        }
      } else {
        // Validate version
        ...
        this.logger.debug(`API version detected: ${this.versioningService.getVersionString(version)}`);
      }

      // Add version information to request context
      (req as any).versionInfo = {
        detected: version ? this.versioningService.getVersionString(version) : 'default',
        supported: this.versioningService.getSupportedVersions().map(v => this.versioningService.getVersionString(v)),
        isDeprecated: version ? this.versioningService.isVersionDeprecated(version) : false,
      };
  ...
  private handleUnsupportedVersion(req: Request, res: Response, version: any) {
    ...
    const supportedVersions = this.versioningService.getSupportedVersions().map(v => this.versioningService.getVersionString(v));
    const latestVersion = this.versioningService.getLatestStableVersion();
    const latestVersionString = latestVersion ? this.versioningService.getVersionString(latestVersion) : null;
