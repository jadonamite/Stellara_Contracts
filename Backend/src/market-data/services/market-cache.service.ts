...
export class MarketCacheService {
    ...
    try {
      const [hits, misses, totalKeys] = await Promise.all([
        ...
        this.redisService.client
          .keys(`${namespace}:*`)
          .then((keys) =>
            keys.filter((k) => !k.includes(':stats:') && !k.includes(':metadata')).length,
          ),
      ]);
      ...
      return { hits, misses, hitRate, totalKeys, namespace };
    } catch (error) {
      this.logger.error(`Error getting cache stats: ${error.message}`, error.stack);
      return { hits: 0, misses: 0, hitRate: 0, totalKeys: 0, namespace };
    }
  }
  ...
  async getOverallStats(): Promise<{
    ...
    try {
      ...
      return { totalHits, totalMisses, hitRate, namespaces };
    } catch (error) {
      this.logger.error(`Error getting overall stats: ${error.message}`, error.stack);
      return { totalHits: 0, totalMisses: 0, hitRate: 0, namespaces: [] };
    }
  }
